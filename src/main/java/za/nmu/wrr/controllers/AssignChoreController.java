...
public class AssignChoreController extends Controller {
    public final static String v = "Assign Chores";
    ...
    public AssignChoreController(Scene dashboardScene,Stage acStage) {
        ...
        linksRunnable.add(() -> setupDashboardLinks(dashboardScene, acStage));
    }
    ...
    private void onDashboardLinkClicked(CircleView cv) {
        ...
        if (cv == null)acStage.showAndWait();
        else if (!cv.dragging()) acStage.showAndWait();
    }
    ...
    private void setupDashboardLinks(Scene dashboardScene, Stage acStage) {
        ...
        if (loggedInUser != null) {
            if (loggedInUser.isLeader.getValue() == 0)
                ...
                avail(hpAssignChore);
                hpAssignChore.setOnAction(event -> onDashboardLinkClicked(null));
            }
        }
    }
